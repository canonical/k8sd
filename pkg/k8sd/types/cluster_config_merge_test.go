...
func TestMergeClusterConfig(t *testing.T) {
	for _, tcs := range [][]mergeClusterConfigTestCase{
		...
		generateMergeClusterConfigTestCases("Network/Patches", true,
			[]types.Patch{{Target: types.PatchTarget{Kind: "DaemonSet", Name: "cilium"}, StrategicMerge: utils.Pointer("spec:\n  foo: bar\n")}},
			[]types.Patch{{Target: types.PatchTarget{Kind: "DaemonSet", Name: "cilium"}, StrategicMerge: utils.Pointer("spec:\n  foo: baz\n")}},
			func(c *types.ClusterConfig, v any) { c.Network.Patches = utils.Pointer(v.([]types.Patch)) }),
