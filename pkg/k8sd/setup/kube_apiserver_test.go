...
func TestKubeAPIServer(t *testing.T) {
	t.Run("ArgsWithProxy", func(t *testing.T) {
		...
		tests := []struct {
			...
			{key: "--etcd-cafile", expectedVal: filepath.Join(s.Mock.EtcdPKIDir, "ca.crt")},
			{key: "--etcd-certfile", expectedVal: filepath.Join(s.Mock.KubernetesPKIDir, "apiserver-etcd-client.crt")},
			{key: "--etcd-keyfile", expectedVal: filepath.Join(s.Mock.KubernetesPKIDir, "apiserver-etcd-client.key")},
	...
	t.Run("IPv6", func(t *testing.T) {
		...
		g.Expect(setup.KubeAPIServer(s, 6443, net.ParseIP("2001:db8::"), "fd98::/108", "https://auth-webhook.url", false, types.Datastore{Type: utils.Pointer("etcd")}, "Node,RBAC", nil)).To(Succeed())
