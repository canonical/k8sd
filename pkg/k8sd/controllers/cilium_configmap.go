...
// CiliumConfigMapController watches the k8sd-cilium-values ConfigMap and triggers
// network reconciliation whenever the ConfigMap is created, updated, or deleted.
type CiliumConfigMapController struct {
	snap          snap.Snap
	notifyNetwork func()
}

// NewCiliumConfigMapController creates a new CiliumConfigMapController.
func NewCiliumConfigMapController(snap snap.Snap, notifyNetwork func()) *CiliumConfigMapController {
	return &CiliumConfigMapController{
		snap:          snap,
...
func (c *CiliumConfigMapController) Run(ctx context.Context) {
	ctx = log.NewContext(ctx, log.FromContext(ctx).WithValues("controller", "cilium-configmap"))
...
func (c *CiliumConfigMapController) watch(ctx context.Context) error {
	log := log.FromContext(ctx)
	...
	log.Info("Started watching k8sd-cilium-values ConfigMap")
