...
func (e *Endpoints) postWorkerInfo(s types.State, r *http.Request) types.Response {
	...
	if nodeIP == nil {
		return types.BadRequest(fmt.Errorf("failed to parse node IP address %s", req.Address))
	}
	...
	if err != nil {
		return types.InternalError(fmt.Errorf("failed to generate worker PKI: %w", err))
	}

	client, err := snap.KubernetesClient("")
	if err != nil {
		return types.InternalError(fmt.Errorf("failed to create kubernetes client: %w", err))
	}
	if err := client.WaitKubernetesEndpointAvailable(r.Context()); err != nil {
		return types.InternalError(fmt.Errorf("kubernetes endpoints not ready yet: %w", err))
	}

	// Check if the node name already exists in the cluster.
	_, err = client.GetNode(r.Context(), workerName)
	if err == nil {
		return types.BadRequest(fmt.Errorf("node name already exists: %s", workerName))
	} else if !apierrors.IsNotFound(err) {
		// Request to fetch node failed for some other reason
		return types.InternalError(fmt.Errorf("failed to check whether worker node name is available %s: %w", workerName, err))
	}

	servers, err := client.GetKubeAPIServerEndpoints(r.Context())
	if err != nil {
		return types.InternalError(fmt.Errorf("failed to retrieve list of known kube-apiserver endpoints: %w", err))
	}
	...
	if err := s.Database().Transaction(r.Context(), func(ctx context.Context, tx *sql.Tx) error {
		...
		return types.InternalError(fmt.Errorf("delete worker node token transaction failed: %w", err))
	}

	return types.SyncResponse(true, &apiv2.GetWorkerJoinInfoResponse{
