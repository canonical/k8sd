...
func newXWaitForCmd(env cmdutil.ExecutionEnvironment) *cobra.Command {
	...
	waitForDNSCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := control.WaitUntilReady(ctx, func() (bool, error) {
				probeResult := features.StatusChecks.CheckDNS(cmd.Context(), env.Snap)
				if probeResult.Err != nil {
					cmd.PrintErrf("DNS not ready yet: %v\n", probeResult.Err.Error())
				}
				return probeResult.Err == nil, nil
			}); err != nil {
	...
	waitForNetworkCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			defer cancel()
			if err := control.WaitUntilReady(ctx, func() (bool, error) {
				probeResult := features.StatusChecks.CheckNetwork(cmd.Context(), env.Snap)
				if probeResult.Err != nil {
